# 🐍 Python Learning Journal - Alexis

¡Hola! Soy estudiante de **Desarrollo de Software en la UTP** y este es mi repositorio personal donde documento mi progreso aprendiendo Python desde cero. 

Aquí guardo mis retos diarios, lógica de programación y experimentos con el lenguaje.

## 🚀 Progreso por Días


## 📁 Contenido del Portafolio
- [Día 1: Fundamentos](practicas_diarias/day1_basics_python/README.md)
- [Día 2: Ciclos For](practicas_diarias/day2_basics_python/README.md)
- [Día 3: Ciclos While](practicas_diarias/day3_basics_python/README.md)
- [Día 4: Diccionarios y Módulos](practicas_diarias/day4_basics_python/README.md)
- [Día 5: Manejo de Archivos y Excepciones](practicas_diarias/day5_basics_python/README.md)
---
🛠️ Infraestructura y DevOps
Este proyecto no es solo código; incluye automatización profesional:

CI (Integración Continua): Implementé GitHub Actions para validar automáticamente la sintaxis de cada script de Python en cada push.

Consistencia: Uso de rutas dinámicas y limpieza de datos con .strip() para asegurar que el Backend sea robusto.
